...
int main() {
	const int ADD_CHOICE = 1,
		...
		REMOVE_CHOICE = 5,
		SEARCH_CHOICE = 6;
	int choice, size = 3, index = 0;
	char name[15];
	Food* fruitPtr = new Food[size];
	Food fruit;
	...
	{
		getValues(fruit);
		EqualOperator(fruitPtr, fruit, index);
	}
	...
	do {
		...
		case REMOVE_CHOICE:
			cin.ignore();
			cout << "Enter the name of the fruit you want to remove: ";
			
			cin.getline(name, 15);
			index = fruit.searchList(fruitPtr, name, size);
			fruit.removeItem(fruitPtr, name, size);
			
			print(fruitPtr, size);
			break;
		case SEARCH_CHOICE:
			cout << "Enter the name of the fruit you want to search: ";
			cin.ignore();
			cin.getline(name, 15);
			index = fruit.searchList(fruitPtr, name, size);
			...
			if (index != -1)
			{
				fruitPtr[index].display();
			}
			else
			{
				cout << "The fruit is not in the list." << endl;
			}
...
int menu() {
	...
	cout << "\n\t\tMenu\n\n"
		...
		<< "6. Search for a fruit in the list\n"
		<< "Enter your choice: ";
	cin >> choice;
	return choice;
}
