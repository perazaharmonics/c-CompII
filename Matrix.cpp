...
void Matrix::subtract(Matrix aMatrix)
{
    Matrix resultingMatrix(getRows(),getCols());
    for (int i = 0; i < getRows(); i++)
    {
        for (int j = 0; j < getCols(); j++)
        {
            resultingMatrix.setValue(getValue(i, j) - aMatrix.getValue(i, j), i, j); // Set the value of the resulting matrix to the difference of the values of the current matrix and aMatrix
        }//end for
    }//end for
    cout<<"Matrix C"<<endl;
    resultingMatrix.display();
}
void Matrix::Hadamard(Matrix aMatrix)
{   Matrix resultingMatrix(getRows(),getCols());
    for (int i = 0; i < getRows(); i++)
    {
        for (int j = 0; j < getCols(); j++)
        {
            resultingMatrix.setValue(getValue(i, j) * aMatrix.getValue(i, j), i, j); // Set the value of the resulting matrix C to the Hadamard product of the values of the current matrix and aMatrix
        }//end for
    } //end for
    cout<<"Matrix C"<<endl;
    resultingMatrix.display();
